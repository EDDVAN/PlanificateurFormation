...
<body class="flex flex-col flex-col md:flex-row">
    ...
    <section class="flex flex-col p-4 md:p-8 gap-4 md:gap-8 w-screen md:w-[calc(100%-14rem)]">
        <div class="flex align-center justify-between gap-2 h-24">
            <h1 class="place-content-center text-3xl text-gray-700">Dates Formations (<?= count($data['old']) + count($data['new']); ?>)</h1>
            <div class="flex flex-col relative align-center justify-center ">
                <a href="/gestion/date/add"
        ...
        <div class="flex flex-col gap-2 w-full h-[calc(100%-4rem)] ">
            <div class="flex flex-col flex-wrap md:flex-nowrap gap-2">
                <form action="/gestion/date" method="get" class="flex gap-2">
            ...
            <div class="flex flex-nowrap gap-2">
                <button id="dates-old-btn" class="px-3 py-2 flex flex-1 rounded-t-md justify-center align-center gap-2 cursor-pointer border 
                            border-emerald-600 text-sm font-medium text-emerald-600 hover:text-white hover:bg-emerald-600"
                    onclick="showTab(0)">
                    Anciennes
                </button>
                <button id="dates-new-btn" class="px-3 py-2 flex flex-1 rounded-t-md justify-center align-center gap-2 cursor-pointer border
                            border-emerald-600 text-sm font-medium text-white bg-emerald-600"
                    onclick="showTab(1)">
                    A venir
                </button>
            </div>
            <div class="dates-old hidden">
                <table class="min-w-full divide-y-2 divide-gray-200 p-2" id="dates-old-table">
                    <thead class="ltr:text-left rtl:text-right">
                        <tr class="*:font-medium *:text-white bg-emerald-600 ">
                            <th class="px-3 py-2 whitespace-nowrap w-2/20">Domaine</th>
                            <th class="px-3 py-2 whitespace-nowrap w-3/20">Sujet</th>
                            <th class="px-3 py-2 whitespace-nowrap w-3/20">Cours</th>
                            <th class="px-3 py-2 whitespace-nowrap w-2/20">Formateur</th>
                            ...
                            <th class="px-3 py-2 !text-left w-2/20">Prix</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        ?>
                            <tr class="*:text-gray-900 *:first:font-medium">
                                ...
                                <td class="px-3 py-2 whitespace-wrap"><?= $row->ville . ', ' . $row->pays; ?></td>
                                ...
                                <td class="px-3 py-2 flex flex-col lg:flex-row align-center justify-around gap-2 border-0 w-fit">
                                    <a href="/gestion/date/edit?id=<?= $row->id ?>" class="cursor-pointer flex w-12 justify-center align-center text-sm gap-1 px-2 py-1.5 text-white bg-amber-500">
                                        <i class="ph ph-pencil text-lg"></i>
                                    </a>
                                    <button
                                        data-id="<?= $row->id ?>"
                                        data-name="<?= $row->cours . ' - ' . $row->formateur ?>"
                                        data-url="/gestion/date/delete"
            ...
            <div class="dates-new">
                <table class="min-w-full divide-y-2 divide-gray-200 p-2 " id="dates-new-table">
                    <thead class="ltr:text-left rtl:text-right">
                        <tr class="*:font-medium *:text-white bg-emerald-600 ">
                            <th class="px-3 py-2 whitespace-nowrap w-2/20">Domaine</th>
                            <th class="px-3 py-2 whitespace-nowrap w-3/20">Sujet</th>
    ...
    <script>
        if ($('#dates-old-table') != null) {
            let table = $('#dates-old-table').dataTable({
                "pageLength": 10,
                stateSave: true,
                autoWidth: false
            });
        }
        if ($('#dates-new-table') != null) {
            let table = $('#dates-new-table').dataTable({
                "pageLength": 10,
                stateSave: true,
                autoWidth: false
            });
        ...
        function showTab(id) {
            if (id == 0) {
                $('.dates-old').removeClass('hidden');
                $('.dates-new').addClass('hidden');

                $('#dates-old-btn').removeClass('text-emerald-600');
                $('#dates-old-btn').addClass('text-white');
                $('#dates-old-btn').addClass('bg-emerald-600');

                $('#dates-new-btn').addClass('text-emerald-600');
                $('#dates-new-btn').addClass('hover:text-white');
                $('#dates-new-btn').addClass('hover:bg-emerald-600');
                $('#dates-new-btn').removeClass('text-white');
                $('#dates-new-btn').removeClass('bg-emerald-600');

            } else {
                $('.dates-old').addClass('hidden');
                $('.dates-new').removeClass('hidden');

                $('#dates-new-btn').removeClass('text-emerald-600');
                $('#dates-new-btn').addClass('text-white');
                $('#dates-new-btn').addClass('bg-emerald-600');

                $('#dates-old-btn').addClass('text-emerald-600');
                $('#dates-old-btn').addClass('hover:text-white');
                $('#dates-old-btn').addClass('hover:bg-emerald-600');
                $('#dates-old-btn').removeClass('text-white');
                $('#dates-old-btn').removeClass('bg-emerald-600');
            }
        }
    </script>
