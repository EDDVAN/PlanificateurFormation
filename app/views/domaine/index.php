...
<body class="flex relative">
    ...
    <section class="flex flex-col p-4 md:p-8 gap-4 md:gap-8 w-[calc(100%-14rem)]">
        <div class="flex align-center justify-between gap-2 h-24">
            <h1 class="place-content-center text-3xl text-gray-700">Domaines (<?= count($data); ?>)</h1>
            <div class="flex flex-col relative align-center justify-center ">
                <a href="/gestion/domaine/add"
        ...
        <div class="flex flex-col gap-2 w-full h-[calc(100%-4rem)]">
            <table class="min-w-full divide-y-2 divide-gray-200 p-2 " id="domaine-table">
                    ...
                    ?>
                        <tr class="*:text-gray-900 *:first:font-medium">
                            ...
                            <td class="px-3 py-2 flex align-center justify-around gap-2 border-0">
                                <a href="/gestion/domaine/edit?id=<?= $row->id ?>" class="cursor-pointer flex w-12 justify-center align-center text-sm gap-1 px-2 py-1.5 text-white bg-amber-500 ">
                                    <i class="ph ph-pencil text-lg"></i>
                                </a>
                                <button
                                    data-id="<?= $row->id ?>"
                                    data-name="<?= $row->name ?>"
                                    data-url="/gestion/domaine/delete"
    ...
    <script>
        if ($('#domaine-table') != null) {
            let table = $('#domaine-table').dataTable({
                "pageLength": 10,
                stateSave: true,
                autoWidth: false
            });
